...
from workers import SmallCellWorker
...
def parse_args():
    ...
    parser.add_argument('--algorithm', type=str, default='reinforce', choices=['reinforce', 'hyperband'])
    ...
    parser.add_argument('--controller-train-paths-per-step',   type=int, default=30)   # Number of paths to use to train controller per step
    ...
    parser.add_argument('--num-ops', type=int, default=5)   # Number of ops to sample
    parser.add_argument('--num-nodes', type=int, default=5) # Number of cells to sample
    
    # RL Parameters
    parser.add_argument('--temperature', type=float, default=1)          # Temperature for logit -- higher means more entropy 
    parser.add_argument('--clip-logits', type=float, default=1.10)       # Clip logits
    parser.add_argument('--entropy-penalty', type=float, default=0.0001) # Penalize entropy 
...
if __name__ == "__main__":
    ...
    set_seeds(args.seed)
    worker = SmallCellWorker(num_nodes=args.num_nodes).cuda()
